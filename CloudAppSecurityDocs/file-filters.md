...
![policy_file 형식 필터](./media/policy_file-type-filters.png "policy_file type filters")  
...
> [!NOTE]
>  외부 사용자에 의해 연결된 저장소 앱으로 공유되는 파일은 Cloud App Security에서 다음과 같이 처리됩니다.     - **OneDrive:** OneDrive는 내부 사용자를 외부 사용자에 의해 OneDrive에 배치된 모든 파일의 소유자로 지정합니다. 그러면 이러한 파일은 조직의 소유로 간주되므로 Cloud App Security에서 이러한 파일을 검색하고 OneDrive의 다른 모든 파일에서처럼 정책을 적용합니다.
     - **Google Drive:** Google Drive는 이러한 파일을 외부 사용자의 소유로 간주하며, 조직이 소유하지 않는 파일 및 데이터에 대한 법적 제한으로 인해 Cloud App Security에서 이러한 파일에 액세스할 수 없습니다.
